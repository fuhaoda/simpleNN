...
class MSE:
    ...
    @staticmethod
    def loss_function(yhat, y):  # <1>
        assert y.ndim == yhat.ndim == 1
        diff = y - yhat
        return 0.5 * sum(diff * diff)

    @staticmethod
    def loss_derivative(yhat, y):  # <2>
        return yhat - y

    # <1> we used the mean squared error loss function, MSE = 1/2 (y-yhat)^2
    # <2> the derivative is an MSE loss
...
class SequentialNetwork:  # <1>
    ...
    def train(self, training_data, epochs, mini_batch_size, learning_rate, test_data=None):
        ...
        for epoch in range(epochs):  # <1>
            ...
            mini_batches = [training_data[k:k + mini_batch_size] for k in range(0, n, mini_batch_size)]  # <2>
    ...
    def forward_backward(self, mini_batch): #todo: improve the codes by training in mini batch directly
        for x, y in mini_batch:
            self.layers[0].input_data = x
            for layer in self.layers:
                layer.forward()  # <4>
            self.layers[-1].input_delta = self.loss.loss_derivative(self.layers[-1].output_data, y)  # <5>
            for layer in reversed(self.layers):
                layer.backward()  # <6>

    # <1> The parameter was updated one case by one case by using += in layers.py. So we need to normalize it by
    # mini-batch size.
    ...
    # <1> Pass a single sample forward and return the result. This will be used for prediction in single case
